...
garden_harvest %>% 
  ...
  group_by(variety) %>%
  mutate(cumulative_harvest_lb = cumsum(daily_harvest_lb)) %>%
  ggplot() +
  geom_area(aes(x = date, y = cumulative_harvest_lb, fill = fct_reorder2(variety, variety, cumulative_harvest_lb,.desc = FALSE))) +
  theme(legend.position = "top") +
  labs(title = "Tomatoes Cumulative Harvest, by Variety",
...
ggmap(mallorcaMap) + 
  geom_path(data = mallorca_bike_day7, 
             ...
             size = .5) +
  annotate("point", x = 2.586712, y = 39.66002, color = "red") +
  annotate("text", x = 2.586712, y = 39.65002, color = "red", 
...
myCovid19 <-
  ...
  filter(cases > 20) %>%
  ggplot(aes(x = cases, y = new_cases_per_week, color = state)) +
  scale_color_viridis_d() +
  geom_path() +
  scale_x_log10() +
  scale_y_log10() +
